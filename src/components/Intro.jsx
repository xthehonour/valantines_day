...
const Intro = ({ onNext, onStartMusic }) => {
  const [isOpen, setIsOpen] = useState(false);
  const introImage = new URL("../assets/fotolar/intro.jpg", import.meta.url).href;
  const handleOpen = () => {
    setIsOpen(true);
    onStartMusic();
  };
  return (
    ...
    >
      <div className="mb-8 cursor-pointer transform hover:scale-110 transition-transform" onClick={handleOpen}>
      ...
      {!isOpen ? (
        ...
        >
          ...
          <p className="text-sm md:text-base leading-relaxed mb-6 text-rose-800">
            Daima beraber olalım canım sevgilimm Sevgililer günümüz kutlu olsun ❤️
          </p>
          <button 
            onClick={onNext}
            className="bg-rose-500 text-white px-6 py-3 rounded-none pixel-border hover:bg-rose-600 transition-colors"
